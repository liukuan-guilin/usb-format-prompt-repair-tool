...
```mermaid
flowchart TD
    A["你现在看到的情况是什么？"] --> B{"Windows 提示需要格式化？"}
    B -- "是" --> C["用 UsbRepairTool.exe"]
    B -- "不是" --> D{"U盘看起来空了，或者文件夹变成快捷方式？"}
    D -- "是" --> E["用 UsbHiddenFileRecovery.exe"]
    D -- "不是" --> F{"文件能看到，但有些名字乱码？"}
    F -- "是" --> G["如果同时有隐藏文件/快捷方式现象，先用隐藏文件恢复工具。<br/>如果内容还能打开，先复制到硬盘再改名。"]
    F -- "不是" --> H["这个仓库可能不完全适用。<br/>继续排查硬件故障或更深层文件系统问题。"]
```
...
如果你完全不想碰命令行，现在最简单的是直接用下面两个桌面版 exe：

- [dist/UsbRepairTool.exe](dist/UsbRepairTool.exe)
  适用于“Windows 提示需要格式化”的场景。
- [dist/UsbHiddenFileRecovery.exe](dist/UsbHiddenFileRecovery.exe)
  适用于“文件被隐藏 / 文件夹变快捷方式 / 文件名乱码”的场景。

其中 `UsbRepairTool.exe` 能做的事：
...
```mermaid
flowchart TD
    A["U盘看起来空了，或者全是快捷方式"] --> B["运行 UsbHiddenFileRecovery.exe"]
    B --> C["去掉 隐藏 / 只读 / 系统 属性"]
    C --> D["可选把根目录可疑文件移到隔离区"]
    D --> E{"文件重新显示了吗？"}
    E -- "是" --> F{"文件名还乱码吗？"}
    F -- "不乱码" --> G["先把重要文件复制出来，再对U盘查毒"]
    F -- "还乱码" --> H["如果内容还能打开，先复制到硬盘再改名"]
    E -- "没有" --> I["改走更深层的恢复或杀毒清理"]
```
